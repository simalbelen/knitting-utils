...
function EditSectionForm({ onClose, section }: Props) {
  ...
  const formik = useFormik({
    initialValues: {
      title: section.title,
      notes: section.notes,
      goal_row: section.goal_row,
      knit_flat: section.knit_mode.knit_flat,
      right_side_even_row: section.knit_mode.right_side_even_row,
    },
    ...
    onSubmit: async (values) => {
      ...
      if (section._id === undefined) {
        toast.error("El id de la sección no está definido");
        return;
      }
      try {
        await SectionService.updateOne(section._id, {
